...
public class Elevador {
    ...
    public void mover(int totalAndares) {
        // Simples lógica: sobe um andar por ciclo até último andar, depois desce
        if (andarAtual < totalAndares - 1) {
            andarAtual++;
        } else {
            andarAtual = 0; // Reinicia no térreo após chegar ao topo (simplificação)
        }
    }

    // Adiciona pessoa ao elevador se houver capacidade
    ...
    public void removerPessoa(Pessoa pessoa) {
        // Remove pessoa da lista — aqui uma remoção simples, iterando
    ...
    // Retorna lista das pessoas que devem sair no andar atual
